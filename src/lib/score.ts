...
export async function calculateEfficiencyScore(userId: string) {
  ...
  const avgCompression =
    jobs.length > 0
      ? jobs.reduce((s: number, j: any) => s + (j.result?.compressionRatio ?? 0), 0) / jobs.length
      : 0;
  ...
  const avgRoutingSaving =
    routingLogs.length > 0
      ? routingLogs.reduce((s: number, l: any) => {
          const pct =
            l.originalCost > 0
  ...
  const totalSaved =
    promptLogs.reduce((s: number, l: any) => s + l.savedDollars, 0) +
    routingLogs.reduce((s: number, l: any) => s + l.savedDollars, 0) +
    contextLogs.reduce((s: number, l: any) => s + l.savedDollars, 0) +
    jobs.reduce((s: number, j: any) => s + j.savings, 0);
