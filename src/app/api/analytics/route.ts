...
export async function GET(req: NextRequest) {
  ...
  // Layer savings
  const layer1Savings = jobs.reduce((s: number, j: any) => s + j.savings, 0);
  const layer2Savings = promptLogs.reduce((s: number, l: any) => s + l.savedDollars, 0);
  const layer3Savings = routingLogs.reduce((s: number, l: any) => s + l.savedDollars, 0);
  const layer4Savings = contextLogs.reduce((s: number, l: any) => s + l.savedDollars, 0);
  const totalSavings  = layer1Savings + layer2Savings + layer3Savings + layer4Savings;

  // Token stats
  const totalOriginalTokens  = promptLogs.reduce((s: number, l: any) => s + l.originalTokens, 0);
  const totalOptimizedTokens = promptLogs.reduce((s: number, l: any) => s + l.optimizedTokens, 0);
  const avgReductionPct      = promptLogs.length > 0
    ? promptLogs.reduce((s: number, l: any) => s + l.reductionPct, 0) / promptLogs.length
    : 0;

  // Routing stats
  const avgComplexity = routingLogs.length > 0
    ? routingLogs.reduce((s: number, l: any) => s + l.complexity, 0) / routingLogs.length
    : 0;
  const routingByModel: Record<string, number> = routingLogs.reduce((acc: Record<string, number>, l: any) => {
  ...
  const avgContextReduction = contextLogs.length > 0
    ? contextLogs.reduce((s: number, l: any) => s + l.reductionPct, 0) / contextLogs.length
    : 0;

  // Recent jobs for the dashboard table
  const recentJobs = jobs.slice(0, 10).map((j: any) => ({
    id:          j.id,
